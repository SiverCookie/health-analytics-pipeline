...
def test_clean_heart_rate_removes_invalid_and_outliers():
    df = pd.DataFrame({
        "timestamp": ["2025-01-01 00:00", "invalid_date", "2025-01-01 00:02"],
        "heart_rate": ["80", "not_a_number", "300"]  # 300 is out of range
    })
...
def test_clean_steps_removes_negative_and_invalid():
    df = pd.DataFrame({
        "timestamp": ["2025-01-01 01:00", None, "2025-01-01 02:00"],
        "steps": ["10", "-5", "not_a_number"]
    })
